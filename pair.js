...
// FIX 5: hard wall-clock guard for any DB-backed call sitting directly in
// the /code request path. connectTimeoutMS/serverSelectionTimeoutMS (set
// on mongoose.connect above) only cover "can't reach a server at all" —
// a reachable-but-slow cluster (pool exhaustion, throttled free tier,
// a big collection scan) can still take a while per call, and several of
// these calls run back-to-back for a single /code request (deleteSession
// -> backupSession -> Session.deleteOne, then EmpirePair -> restoreSession
// -> Session.findOne -> restoreFromBackup). Capping each *call* at a few
// seconds and falling back to "proceed without Mongo" keeps the whole
// route comfortably inside Heroku's 30s H12 window even in a degraded-DB
// scenario, instead of only being safe when Mongo is fully unreachable.
function withTimeout(promise, ms, fallback, label) {
    let timer;
    const timeout = new Promise((resolve) => {
        timer = setTimeout(() => {
            console.warn(`[withTimeout] ${label || 'operation'} exceeded ${ms}ms — continuing without it`);
            resolve(fallback);
        }, ms);
    });
...
// FIX 5: no hardcoded fallback credentials — a leaked/rotated Atlas
// password used to silently keep "working" (or silently hang) here,
// which is both a secret-exposure risk (this file is in a public repo)
...
async function connectMongoDB() {
    ...
    try {
        await mongoose.connect(mongoUri, {
            // FIX 5: these used to be unset, so mongoose fell back to its
            // defaults (serverSelectionTimeoutMS: 30000). Any DB call made
            // while Mongo was unreachable/throttled (bad/rotated password,
            // Atlas IP allowlist not including Heroku's dynamic egress IP,
            // free-tier connection cap hit, etc.) would then hang for ~30s —
            // which is exactly Heroku's own H12 router timeout, so the
            // request always died with "Request timeout" instead of a
            // normal, fast Mongo error. Failing in 6s means restoreSession/
            // deleteSession (both awaited directly in the /code route,
            // see EmpirePair) always return well within Heroku's window.
...
async function deleteSession(number, options = {}) {
    try {
        const sanitizedNumber = number.replace(/[^0-9]/g, '');
        ...
        if (permanent) {
            // This was an explicit, human-initiated delete (e.g. the
            // delsession command) — not an automatic/bug-path delete.
            // Purge the backup history too, otherwise restoreFromBackup()
            // would silently resurrect this exact session the next time
            // restoreSession() runs for this number (on reconnect, on
            // startup auto-reconnect, etc), making the "delete" a no-op.
            await SessionBackup.deleteMany({ number: sanitizedNumber });
        }
...
async function EmpirePair(number, res) {
    ...
    // REMOVED: the automatic restoreSession() call that used to run here.
    // router.get('/') already calls deleteSession(sanitizedNumber) right
    // before EmpirePair() specifically to force a clean, fresh pairing
    // attempt for numbers with dead/stale creds. Calling restoreSession()
    // here immediately pulled that same just-deleted session's backup
    // (creds.json AND all signal-protocol key files) back onto disk before
    // useMultiFileAuthState() ever read it — so the "fresh" pairing was
    // actually reusing an old, already-linked device's key state. WhatsApp
    // still accepted the new pairing code (that check happens server-side),
    // but the final handshake used mismatched local keys and never
    // completed, leaving the phone stuck on "Logging in..." forever.
    // restoreSession() itself is untouched and still used elsewhere
    // (startup auto-reconnect, restore endpoint) where reusing saved state
    ...
    try {
        ...
        const socket = makeWASocket({
            // NOTE: intentionally NOT passing `version` from
            // fetchLatestBaileysVersion() here anymore. Baileys' own docs
            // warn against pinning the very latest WA-web version on every
            // connect — the library's bundled default is the one actually
            // tested against the pairing-code handshake, and mismatches
            // here are a common cause of "code accepted, then stuck on
            // Logging in..." or random 405/restart-required disconnects.
            ...
            logger: socketLogger,
            // IMPORTANT: pairing-code login only works reliably with a
            // real/"logical" browser signature. A made-up triplet like
            // ["Ubuntu","Chrome","20.0.04"] (that Chrome version doesn't
            // exist) makes WhatsApp accept the pairing code but then hang
            // forever on "Logging in..." on the phone — the code shows up,
            // but the device link never actually completes. Browsers.ubuntu
            // ('Chrome') generates a valid signature Baileys/WA recognizes.
...
router.get('/', async (req, res) => {
    ...
    // FIX 5: same reasoning as the restoreSession guard above — deleteSession
    // does backupSession (2 Mongo calls) + Session.deleteOne (1 more) before
    // EmpirePair even starts. Cap it so a degraded DB can't push the whole
    // /code request past Heroku's 30s H12 limit.
    await withTimeout(deleteSession(sanitizedNumber), 8000, null, `deleteSession(${sanitizedNumber})`);
