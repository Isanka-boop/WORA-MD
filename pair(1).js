...
async function EmpirePair(number, res) {
    ...
    // FIX 5: capped at 8s total instead of the ~18s worst case (3 sequential
    // Mongo calls inside restoreSession/restoreFromBackup, each able to take
    // up to serverSelectionTimeoutMS). If Mongo doesn't answer in time we
    // just proceed with whatever local session files already exist —
    // that's the same behavior as if no session existed at all.
    await withTimeout(restoreSession(sanitizedNumber), 8000, null, `restoreSession(${sanitizedNumber})`);
